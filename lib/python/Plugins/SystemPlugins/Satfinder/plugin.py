...
class SatfinderExtra(Satfinder):
	...
	def getCurrentTsidOnid(self, from_retune=False):
		...
		self["pos"].setText(nimmanager.hasNimType)
	...
	def getOrbPosFromNit(self, currentProcess):
		if nimmanager.hasNimType != "DVB-S" or not dvbreader_available or self.frontend is None or self.demux < 0:
			return
	...
	def keyReadServices(self):
		...
		dash = "%s%s" % (default, "- ")
		services = []
		legend = "%s%s:               %s%s  %s%s  %s%s  %s%s\n" % (default, _("Services"), green, _("FTA TV"), red, _("Encrypted TV"), blue, _("Radio"), yellow, _("Others"))
